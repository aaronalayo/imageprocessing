...
class ImageProcessor(Frame):
    """Image processor app
    features: Tkinter interface, Open, Save, Quit, Undo, Add filters, Manipulate Image, Face detection
    Arguments:
        Frame {[Tkinter]} -- The tkinter package (“Tk interface”) is the standard Python interface to the Tk GUI toolkit. 
    """
    ...
    def open_img(self):
        ...
        self.cv2img = cv2.resize(
            self.cv2img, dim, interpolation=cv2.INTER_AREA)
    ...
    def face_detect(self):
        ...
        for (x, y, w, h) in faces:
            self.cv2img = cv2.rectangle(
                self.cv2img, (x, y), (x+w, y+h), (255, 0, 0), 2)
